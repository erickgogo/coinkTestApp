...
const payload = TripleDes.encrypt(encryptedArray, keyHash, { mode: CryptoJs.mode.ECB, padding:
CryptoJs.pad.Pkcs7 });
...
const keyHash = this.getKeyHash(key);
const serialized_json = TripleDes.decrypt({ ciphertext: encryptedArray }, keyHash, { mode: CryptoJs.mode.ECB,
